...
void rdda_run (void *ifnameptr) {
    ...
    while (!done) {
        ...
        printf("tg_pos[0]: %+d, pos[0]: %+2.4lf, vel[0]: %+2.4lf, pre[0]: %+2.4lf, tau_off[0]: %+2.4lf, act_tau[0]: %+2.4lf, tg_pos[1]: %+d, pos[1]: %+2.4lf, vel[1]: %+2.4lf, pre[1]: %+2.4lf, tau_off[1]: %+2.4lf\r",
               ecatSlaves->bel[0].out_motor->tg_pos, rdda->motor[0].motorIn.act_pos-rdda->motor[0].init_pos, rdda->motor[0].motorIn.act_vel, rdda->psensor.analogIn.val1, rdda->motor[0].motorOut.tau_off, rdda->motor[0].motorIn.act_tau,
               ecatSlaves->bel[1].out_motor->tg_pos, rdda->motor[1].motorIn.act_pos-rdda->motor[1].init_pos, rdda->motor[1].motorIn.act_vel, rdda->psensor.analogIn.val2, rdda->motor[1].motorOut.tau_off
        );
