...
void rdda_update(ecat_slaves *ecatSlaves, Rdda *rdda) {
    ...
    int16 pressure_offset_adr1 = 200;
    int16 pressure_offset_adr2 = 300;
    ...
    rdda->psensor.analogIn.val1 = (double)(ecatSlaves->bel[0].in_motor->analog_in + pressure_offset_adr1) * ecatSlaves->bel[0].pascal_per_count * ecatSlaves->bel[0].nm_per_pascal * (-1.0);
    for (int i = 1; i < 2; i++) {
        ...
        rdda->motor[i].motorIn.act_tau = -1.0 * (double)(ecatSlaves->bel[i].in_motor->act_tau) / ecatSlaves->bel[i].units_per_nm;
    ...
    rdda->psensor.analogIn.val2 = (double)(ecatSlaves->bel[1].in_motor->analog_in - ecatSlaves->el3102.in_analog->val1*10*1000/32768 + pressure_offset_adr2) * ecatSlaves->bel[1].pascal_per_count * ecatSlaves->bel[1].nm_per_pascal * (-1.0);
    ...
    for (int j = 1; j < 2; j++) {
        //ecatSlaves->bel[j].out_motor->ctrl_wd = 0;
        ecatSlaves->bel[j].out_motor->tg_pos = (int32)saturation(limit_int32, ecatSlaves->bel[j].init_pos_cnts + (int32)saturation(limit_int32, -1.0 * rdda->motor[j].motorOut.tg_pos * ecatSlaves->bel[j].counts_per_rad));
        ecatSlaves->bel[j].out_motor->vel_off = (int32)saturation(limit_int32, -1.0 * rdda->motor[j].motorOut.vel_off * ecatSlaves->bel[j].counts_per_rad_sec);
